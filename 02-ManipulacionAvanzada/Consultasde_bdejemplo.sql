...
--Seleccionar los nombres de los reprecentantes y las oficinas en donde trabajan
CREATE OR ALTER VIEW vw_oficinas_representantes
AS
SELECT r.Nombre, r.Ventas AS [ventas representantes], o.Ciudad, o.Region, o.Ventas AS [ventasoficinas]
...
--SELECCIONAR LOS PEDIDOS CON FECHA DE IMPOTE, EL NOMBRE DEL REPRECENTANTE
...
SELECT Nombre AS [Nombre Representante], Cuota, Ventas
FROM Representantes 
WHERE Cuota > 200000 AND Ventas < Cuota
ORDER BY [Nombre Representante] DESC;
...
WHERE YEAR(p.Fecha_Pedido) = 1989;

--Muestra la Ciudad de la oficina, la Empresa (cliente) 
--y la Suma de los Importes de sus pedidos. Solo queremos 
--ver los resultados de la ciudad de 'Madrid'
SELECT 
    o.Ciudad, 
    c.Empresa, 
    SUM(p.Importe) 
